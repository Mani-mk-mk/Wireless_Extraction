...
class LoginWindow(QMainWindow):
    def __init__(self):
        ...
        #pages
        self.page_controller = self.findChild(QStackedWidget, 'login_signup_stack')
        ...
        self.signup_page_btn.clicked.connect(lambda: self.page_controller.setCurrentIndex(self.signup_page_index))
        self.login_page_btn.clicked.connect(lambda: self.page_controller.setCurrentIndex(self.login_page_index))
    ...
    def login_user(self):
        ...
        self.query.exec_("select * from user where username = '%s' and password = '%s';" %(self.username, password))
        self.query.first()
        if (
    ...
    def signup_user(self):
        ...
        if self.query.value("username") is not None:
            self.signup_status.setText("A user with that username already exists.")
            self.signup_status.setStyleSheet("color: red; font-weight: bold;")
            return False
        ...
        if password != confirm:
            ...
            self.signup_status.setText("Passwords do not match. Please try again.")
            self.signup_status.setStyleSheet("color: red; font-weight: bold;")
            return False
        try: 
            self.query.exec_(f"insert into user (username, email, password) values ('{username}', '{email}', '{password}');")
        except Exception as e:
            print(e)

        print("User created successfully")
        self.signup_status.setText("Sign Up Successful! Please login to continue. ")
